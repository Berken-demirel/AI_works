...
# simplify some methods and error-checking in FactorGraph for efficiency
    ...
    def get_cardinality(self, variable_name):
...
def make_debug_graph():
    ...
    phi2 = FastDiscreteFactor(['x2', 'x3', 'x4'], [3, 2, 2], np.array([0.2, 0.25, 0.70, 0.30,
                                                                   0.4, 0.25, 0.15, 0.65,
                                                                   0.4, 0.50, 0.15, 0.05]))
    phi3 = FastDiscreteFactor(['x3'], [2], np.array([0.5, 
                                                 0.5]))
    phi4 = FastDiscreteFactor(['x4'], [2], np.array([0.4, 
                                                 0.6]))
...
def make_alarm_graph_partC():
    ...
    G = add_evidence(G, 'MINVOL', 1)
    G = add_evidence(G, 'PRESS', 3)
    
    return G

def add_evidence(G, var, val):
    '''
    ...
    value : value of variable evidence 
    ...
    '''
    var_dim = G.get_cardinality(var)
    # iterate factor neighbors
    fac_nbrs = deepcopy(list(G.neighbors(var)))
    ...
    G.add_factors(phi)
    G.add_nodes_from([phi])
    G.add_edges_from([(var, phi)])
            
    return G

def marg_brute_force(G):
    '''
    MARG_BRUTE_FORCE - Compute marginals by brute force enumeration.  This
...
def run_loopy_bp_parallel( G, max_iters, conv_tol ):
    ...
    Returns
    -------
    nodeMargs : list of marginal distribution of each variable
    ...
    while not done:
        ...
        if iters > 1:
            done = check_converged(M, M_old, conv_tol)
        done = done or (iters == max_iters)
